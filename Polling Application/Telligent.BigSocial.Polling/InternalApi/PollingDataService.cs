...
namespace Telligent.BigSocial.Polling.InternalApi
{
	internal static class PollingDataService
	{
		...
		internal static void AddUpdatePoll(Poll poll)
		{
			using (var connection = GetSqlConnection())
			{
				using (var command = CreateSprocCommand("[polling_Poll_AddUpdate]", connection))
				{
					...
					command.Parameters.Add("@GroupId", SqlDbType.Int).Value = poll.GroupId;
		...
		internal static PagedList<Poll> ListPolls(int groupId, int pageSize, int pageIndex)
		{
			...
			using (var connection = GetSqlConnection())
			{
				using (var command = CreateSprocCommand("[polling_Poll_GetSet]", connection))
				{
		...
		internal static void DeletePollsByGroup(int groupId)
		{
			using (var connection = GetSqlConnection())
			{
		...
		private static Poll PopulatePoll(IDataReader reader)
		{
			...
			poll.GroupId = Convert.ToInt32(reader["GroupId"]);

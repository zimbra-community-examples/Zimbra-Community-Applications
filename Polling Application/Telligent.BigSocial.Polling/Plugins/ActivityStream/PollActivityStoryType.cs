...
{
	public class PollActivityStoryType : IPlugin, ITranslatablePlugin, IActivityStoryType
	{
		...
		public bool CanDeleteStory(Guid storyId, int userId)
		{
			...
			return InternalApi.PollingService.CanModerate(story.ContentId.Value, userId);
		}
		...
		public string GetPreviewHtml(IActivityStory story, Target target)
		{
			...
			return string.Format(_translation.GetLanguageResourceValue("activity_preview_format"),
				...
				poll.Name,
				TEApi.Html.EncodeAttribute(InternalApi.PollingService.PollUrl(poll.Id))
				);			
		}
